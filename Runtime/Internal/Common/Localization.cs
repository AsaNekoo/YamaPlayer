...
namespace Yamadev.YamaStream
{
    public class Localization : ObjectClass
    {
        public static Localization Initialize(string translation)
        {
            ...
            if (VRCJson.TryDeserializeFromJson(translation, out DataToken data) && data.TokenType == TokenType.DataDictionary)
            {
                result[0] = data.DataDictionary;
            }
            else
            {
                result[0] = new DataDictionary();
            }
    ...
    public static class LocalizationExtentions
    {
        public static string GetValue(this Localization i18n, string key)
        {
            ...
            if (!Utilities.IsValid(data)) return key;
            if (data.TryGetValue(language, out var tr) && tr.DataDictionary.TryGetValue(key, out var value))
            {
                return value.String;
            }
            return key;
